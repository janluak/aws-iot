...
def test_parent_function():
    from aws_iot_handler.thing import _is_parent_function
...
def test_delete_values_delta_none():
    from aws_iot_handler.thing import _delete_values_if_present
...
def test_delete_values_delete_different_nested_still_diff():
    ...
    assert _delete_values_if_present(origin, compare) == {"key1": {"sub_key3": "value4"}}
...
def test_delete_values_delete_nested_list():
    ...
    assert _delete_values_if_present(origin, compare) == {"key1": {
        "sub_key1": ["value1", "value2"], "sub_key3": "value4"}
    }
...
def test_update_state_from_empty():
    ...
    assert _update_state_from_response(old_reported_state, update_reported_state) == update_reported_state
...
def test_update_state_new_keys():
    ...
    assert _update_state_from_response(old_reported_state, update_reported_state) == {"key": "value", "new_key": "value"}
...
def test_update_state_update_keys():
    ...
    assert _update_state_from_response(old_reported_state, update_reported_state) == {"key": "new_value"}
...
def test_update_state_update_some_keys():
    ...
    assert _update_state_from_response(
        old_reported_state, update_reported_state
    ) == {"key": "new_value", "next_key": "next_value"}


def test_update_state_update_nested_keys():
    from aws_iot_handler.thing import _update_state_from_response

    old_reported_state = {"key": {"sub_dict": {"sub_sub1": "value", "sub_sub2": "value"}}, "next_key": "next_value"}
    update_reported_state = {"key": {"sub_dict": {"sub_sub1": "new_value"}}}

    assert _update_state_from_response(
        old_reported_state, update_reported_state
    ) == {"key": {"sub_dict": {"sub_sub1": "new_value", "sub_sub2": "value"}}, "next_key": "next_value"}
...
def test_delete_state_update_nested_keys():
    ...
    assert _update_state_from_response(
        old_reported_state, update_reported_state
    ) == {"key": {"sub_dict": {"sub_sub2": "value"}}, "next_key": "next_value"}
...
def test_delete_all_reported():
    ...
    assert _update_state_from_response(old_reported_state, update_reported_state) == dict()
...
#     from aws_iot_handler.thing import _set_values_to_None_if_not_in_present
#
...
def test_update_nested_key_reassignment():
    from aws_iot_handler.thing import _update_nested_dict
